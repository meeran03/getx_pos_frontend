...
import {TextField,InputLabel,makeStyles,FormControl,Select,MenuItem} from '@material-ui/core' 
...
import { updateStoreArea } from 'Services/Stores';
...
export default function UpdateStoreArea(props) {
    ...
    React.useEffect(() => {
        if (props.open) {
          ...
          try {
            ...
            getAreas(item.subarea_detail.area_detail.area_detail.id).then(res => setAreas(res));
            getSubAreas(item.subarea_detail.area_detail.id).then(res => setSubAreas(res));
            let arr;
            getDeliveryBoys().then((res) => {
              arr = res;
              // if (item.delivery_boy_detail != null) {
              //   arr = arr.push(item.delivery_boy_detail)
              // }
              setRiders(res);
            })
            console.log(riders)
            setCity(item.subarea_detail.area_detail.area_detail.id);
            setArea(item.subarea_detail.area_detail.id);
            setSubArea(item.subarea_detail.id);
            setRider(item.delivery_boy)
            console.log(item)
    ...
    <GridContainer>
          <Card>
            <CardHeader color="primary">
              ...
              <p className={classes.cardCategoryWhite}>Update Store Area</p>
            </CardHeader>
            <CardBody>
